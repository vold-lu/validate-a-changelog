...
func TestParseEmptyChangelog(t *testing.T) {
	r := strings.NewReader("")
	_, err := Parse(r)
	if err == nil {
		t.Fatal()
...
func TestParseInvalidChangelog(t *testing.T) {
...
func TestParseValidChangelog(t *testing.T) {
	...
	// Make sure the version are parsed in the good order
	if c.Versions[0].Version != "Unreleased" {
		t.Logf("Expected Unreleased version in c.Versions[0]. Got: %s", c.Versions[0].Version)
		t.Fail()
	}
	if c.Versions[14].Version != "0.0.1" {
		t.Logf("Expected Unreleased version in c.Versions[0]. Got: %s", c.Versions[14].Version)
		t.Fail()
	}
